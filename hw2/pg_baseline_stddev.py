...
def train_baseline_model(model, observations, returns, args):
    ...
    #return model.train_on_batch(observations, returns)
    history = model.fit(observations, returns, batch_size=args.baseline_batch_size, verbose=False)
    return history.history['loss'][-1]


def predict_baselines(model, observations):
    return model.predict_on_batch(observations)[:, 0]
...
if __name__ == '__main__':
    ...
    for e in range(args.n_experiments):
        ...
        csvfile.close()
        model_policy.save(os.path.join(logdir, 'model.hdf5'))

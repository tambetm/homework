...
class Histogram(Density_Model):
    ...
    def get_count(self, states):
        ...
        """
        counts = [self.hist[self.preprocessor(state)] for state in states]
        return counts
    ...
    def get_prob(self, states):
        ...
        """
        probs = [self.hist[self.preprocessor(state)] / float(self.total) for state in states]
        return probs
...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            # 1. Compute deltas
            deltas = states[:, np.newaxis] - self.means[np.newaxis]
            ...
            # Gaussian
            # TODO: Skip denominator, because no need to normalize?
            gaussians = np.exp(-euc_dists / (2 * (self.sigma**2))) / np.sqrt(2 * np.pi * (self.sigma**2))
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        print("self.discrim_target:", self.discrim_target)
        print("self.discriminator:", self.discriminator)
        self.log_likelihood = tf.squeeze(self.discrim_target * tf.log(self.discriminator.prob(1)) + (1 - self.discrim_target) * tf.log(self.discriminator.prob(0)), axis=1)
        self.likelihood = tf.squeeze(self.discrim_target * self.discriminator.prob(1) + (1 - self.discrim_target) * self.discriminator.prob(0), axis=1)
        self.kl = self.encoder1.kl_divergence(self.prior) + self.encoder2.kl_divergence(self.prior)
        print("self.log_likelihood", self.log_likelihood)
        print("self.likelihood", self.likelihood)
        print("self.kl", self.kl)
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        with tf.variable_scope(scope):
            z_logstd = tf.Variable(tf.zeros((z_size,)), trainable=True, name='z_logstd')
